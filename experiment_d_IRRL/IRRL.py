...
class IRRL_actor_disturber_critic:
    """
    ...
    def __init__(self, **kwargs):    
        ...
        self.exploration = kwargs.get('exploration', np.identity(self.action_dim,dtype=np.float32))
        self.consistency = kwargs.get('consistency', np.identity(self.action_dim,dtype=np.float32))
        self.shaping = kwargs.get('shaping', np.zeros((self.action_dim,1)))
        ...
        self.hidden_units_actor = [self.state_dim, self.hidden_units[1], self.action_dim]
        self.hidden_units_disturber = [self.state_dim, self.hidden_units[2], self.disturbance_dim]
...
#        np.random.seed(1234) # seed
        ...
        self.advice, self.state, self.value, self.optimizer_critic, self.theta, self.action, self.mean, self.log_policy, self.optimizer_actor, self.action_choose, self.upsilon, self.disturbance, self.mean_disturber, self.log_generator, self.optimizer_disturber, self.disturbance_choose, self.variables_disturber = self._actor_disturber_critic_model_()
...
#        config.gpu_options.per_process_gpu_memory_fraction = 1
    ...
    def _actor_disturber_critic_model_(self):
        ...
        action = tf.placeholder('float', [None, self.action_dim], 'action')
        ...
        with tf.name_scope('Actor'):
            ...
            with tf.name_scope('variables'):
                variables_actor = {
                    'upsilon_ker': tf.Variable(tf.random_normal(self.hidden_units_actor[0:2]), #[0,1]
                                        name='upsilon_ker'),
                    'upsilon_b_ker': tf.Variable(tf.random_normal([self.hidden_units_actor[1]]),
                                      name='upsilon_b_ker'),
                    'upsilon_out': tf.Variable(tf.random_normal(self.hidden_units_actor[1:3]), #[1,2]
                                       name='upsilon_out'),
                    'upsilon_b_out': tf.Variable(tf.random_normal([self.hidden_units_actor[2]]), 
                                                 name='upsilon_b_out')
                }
            ...
            with tf.name_scope('advice'):
                V_advice = tf.abs(advice)*np.linalg.inv(self.consistency) + np.linalg.inv(self.exploration)
                Delta_A = tf.matmul(np.linalg.inv(self.exploration),
                                         tf.reshape(mean, (self.action_dim,1))) 
                Delta_B = np.matmul(np.linalg.inv(self.consistency), self.shaping)
                Delta_advice = Delta_A + Delta_B
                Sigma = tf.linalg.inv(tf.reshape(V_advice,(self.action_dim, self.action_dim)))
                Mean_j = tf.reshape(tf.matmul(Sigma, Delta_advice), (1, self.action_dim))
#                print(Mean_j)

            policy = tfp.distributions.MultivariateNormalFullCovariance(
                    loc = Mean_j, covariance_matrix = Sigma)
            log_policy = tf.squeeze(policy.log_prob(action))
            
            # Training and optimize the policy
            with tf.name_scope('training'):
                rate_scale_williams = self.alpha_upsilon * tf.trace(tf.linalg.inv(Sigma))/self.action_dim
        ...
        with tf.name_scope('Disturber'):
            ...
            with tf.name_scope('variables'):
                variables_disturber = {
                    'omega_ker': tf.Variable(tf.random_normal(self.hidden_units_disturber[0:2]), 
                        name='omega_ker'),
                    'omega_out': tf.Variable(tf.random_normal(self.hidden_units_disturber[1:3]),
                        name='omega_out'),
                    'omega_b_ker': tf.Variable(tf.random_normal([self.hidden_units_disturber[1]]),
                        name='omega_b_ker'),
                    'omega_b_out': tf.Variable(tf.random_normal([self.hidden_units_disturber[2]]), 
                        name='omega_b_out')
                }
        ...
        return advice, state, value, optimizer_critic, variables_critic, action, Mean_j, log_policy, optimizer_actor, action_choose, variables_actor, disturbance, mean_disturber, log_generator, optimizer_disturber, disturbance_choose, variables_disturber
    ...
    def learn(self, state, action, advice, disturbance, reward, next_state):
...
#        env.seed(1234) # seed
        ...
        if self.RL_parameters is None:
            agent = IRRL_actor_disturber_critic()
        else:
            agent = IRRL_actor_disturber_critic(**self.RL_parameters)
        # end if
        ...
        for i in range(self.episodes):
            ...
            while True:
                ...
                # Update the RL model
                TD, log_policy, log_generator = agent.learn(self._state_normalize_(Xt), np.expand_dims([Ut],0).reshape((1,agent.action_dim)), Jt, np.expand_dims(Wt,0), Rt, self._state_normalize_(Xt_))
                ...
                if done == True or iterations[i] >= self.cut:
                    parameter_ = {'robustness': agent.robustness, 'exploration': agent.exploration,
